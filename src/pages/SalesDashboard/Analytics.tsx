    ...
    return (
        <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{t('salesDashboard.analytics.title')}</h2>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">
                        {t('salesDashboard.analytics.subtitle')}
                    </p>
                </div>
                <div className="flex gap-2">
                    <select className="h-10 px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm outline-none">
                        <option>{t('salesDashboard.analytics.last30Days')}</option>
                        <option>{t('salesDashboard.analytics.thisQuarter')}</option>
                        <option>{t('salesDashboard.analytics.thisYear')}</option>
                    </select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                    { label: t('salesDashboard.analytics.winRate'), value: "24.5%", trend: "+2.1%", icon: Target, isPositive: true },
                    { label: t('salesDashboard.analytics.avgDealSize'), value: "$4,250", trend: "+$450", icon: Briefcase, isPositive: true },
                    { label: t('salesDashboard.analytics.salesCycle'), value: "18 Days", trend: "-2 Days", icon: TrendingUp, isPositive: true },
                    { label: t('salesDashboard.analytics.lostDeals'), value: "12", trend: "+3", icon: Users, isPositive: false },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm min-h-[350px] flex flex-col">
                    <h3 className="text-base font-bold text-slate-900 dark:text-white mb-6">{t('salesDashboard.analytics.revenueForecast')}</h3>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm min-h-[350px] flex flex-col">
                    <h3 className="text-base font-bold text-slate-900 dark:text-white mb-6">{t('salesDashboard.analytics.leadsByIndustry')}</h3>
                    <div className="flex-1 w-full h-full min-h-[250px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm min-h-[400px] flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-base font-bold text-slate-900 dark:text-white">{t('salesDashboard.analytics.conversionFunnel')}</h3>
                        <Badge className="bg-brand/10 text-brand border-none">{t('salesDashboard.analytics.last30Days')}</Badge>
                    </div>
                    <div className="flex-1 bg-slate-50/50 dark:bg-slate-800/20 rounded-lg flex items-center justify-center p-6">
                        <div className="w-full max-w-2xl">
                            ...
                            <div className="space-y-4 w-full">
                                {[
                                    { step: t('salesDashboard.analytics.prospectsFound'), count: 1420, pct: "100%" },
                                    { step: t('salesDashboard.analytics.leadsSaved'), count: 350, pct: "24.6%" },
                                    { step: t('salesDashboard.analytics.demosSent'), count: 125, pct: "8.8%" },
                                    { step: t('salesDashboard.analytics.proposals'), count: 45, pct: "3.1%" },
                                    { step: t('salesDashboard.analytics.wonDeals'), count: 18, pct: "1.2%" },
                                ].map((row, i) => (
