...
| D24 | Admin consolidation scope | The 55-component admin is audited in P3 into **Mission Control**: keep + extend (PlansManager, PricingManager, LeadsManager, AiAgentManager, BrainAgentDashboard, analytics, QuotesManager) · merge duplicates (Enhanced* vs base variants collapse to one each) · kill template-era managers with the P0 amputation · new panels (Tenants, Provisioning queue/gate, Change-request queue, Industry-pack editor). One sidebar, one design language, split bundle per D8. |
