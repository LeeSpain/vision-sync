...
-- Drop existing policies for customer_interactions and recreate
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Enable read access for all users" ON customer_interactions;
  DROP POLICY IF EXISTS "Enable insert access for all users" ON customer_interactions;
END $$;
...
-- Create or replace trigger function for quotes
...
-- Create or replace function to calculate lead score
